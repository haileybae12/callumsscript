--[[
    Callum's Script Hub | Aimbot Rev. 2
    
    Change Log:
    - [FIX] Dropdown menus for "Aim Part" and "Target Player" were being parented incorrectly,
      causing them to be clipped and appear blank. They are now parented to the main ScreenGui
      to ensure they render on top of all other elements.
      
    - [FEAT] Added a new "Smoothing" slider. This allows you to control the camera's aim speed,
      transitioning from an instant snap to a smooth, gradual aim. A value of 0.00 is instant,
      while higher values increase the smoothing effect.
      
    - [REFACTOR] Improved the input handling for UI sliders. Both the FOV and new Smoothing
      slider now use a more robust and centralized input-handling system to prevent conflicts.
]]
...
MainWindow.Size = UDim2.new(0, 520, 0, 400) -- Increased height for new slider
...
MinimizeButton.MouseButton1Click:Connect(function()
    ...
    if isMinimized then
        ...
        MainWindow.Size = UDim2.new(0, 520, 0, 400) -- Adjusted height
        MinimizeButton.Text = "-"
    end
end)
...
do
    ...
    local fovSliderTrack = Instance.new("Frame", LeftColumn)
    fovSliderTrack.Size, fovSliderTrack.BackgroundColor3 = UDim2.new(1, 0, 0, 4), Color3.fromRGB(20,20,30)
    fovSliderTrack.BorderSizePixel = 0
    makeUICorner(fovSliderTrack, 2)

    local fovSliderHandle = Instance.new("TextButton", fovSliderTrack)
    fovSliderHandle.Size, fovSliderHandle.Position = UDim2.new(0, 12, 0, 12), UDim2.new(0, 0, 0.5, -6)
    fovSliderHandle.BackgroundColor3, fovSliderHandle.BorderSizePixel = Color3.fromRGB(180, 220, 255), 0
    fovSliderHandle.Text = ""
    makeUICorner(fovSliderHandle, 6)

    -- [NEW] Smoothing UI
    createSectionHeader(LeftColumn, "Smoothing")
    local smoothRow = createSettingRow(LeftColumn, "Smoothness")
    local smoothValueLabel = Instance.new("TextLabel", smoothRow)
    smoothValueLabel.Size, smoothValueLabel.Position = UDim2.new(0.6, 0, 1, 0), UDim2.new(0.4, 0, 0, 0)
    smoothValueLabel.BackgroundTransparency, smoothValueLabel.TextColor3 = 1, Color3.fromRGB(255,255,255)
    smoothValueLabel.Font, smoothValueLabel.TextSize = Enum.Font.Code, 15
    smoothValueLabel.TextXAlignment, smoothValueLabel.TextYAlignment = Enum.TextXAlignment.Left, Enum.TextYAlignment.Center
    
    local smoothSliderTrack = Instance.new("Frame", LeftColumn)
    smoothSliderTrack.Size, smoothSliderTrack.BackgroundColor3 = UDim2.new(1, 0, 0, 4), Color3.fromRGB(20,20,30)
    smoothSliderTrack.BorderSizePixel = 0
    makeUICorner(smoothSliderTrack, 2)
    
    local smoothSliderHandle = Instance.new("TextButton", smoothSliderTrack)
    smoothSliderHandle.Size, smoothSliderHandle.Position = UDim2.new(0, 12, 0, 12), UDim2.new(0, 0, 0.5, -6)
    smoothSliderHandle.BackgroundColor3, smoothSliderHandle.BorderSizePixel = Color3.fromRGB(180, 220, 255), 0
    smoothSliderHandle.Text = ""
    makeUICorner(smoothSliderHandle, 6)
    ...
    partDropdown.MouseButton1Click:Connect(function()
        ...
        dropdownOpen = true
        dropdownFrame = Instance.new("Frame", MainScreenGui) -- [FIX] Parent to MainScreenGui
        local absolutePos = partDropdown.AbsolutePosition
        dropdownFrame.Size = UDim2.new(0, partDropdown.AbsoluteSize.X, 0, #parts * 22)
        dropdownFrame.Position = UDim2.fromPixel(absolutePos.X, absolutePos.Y + 22)
    ...
    local fovRadius = 150
    local aimSmoothing = 0.2 -- [NEW] 0 is instant, 1 is max smoothing.
    ...
    local function updateFovFromHandlePosition()
        local trackWidth = fovSliderTrack.AbsoluteSize.X
        local handleX = fovSliderHandle.Position.X.Offset
        local ratio = math.clamp(handleX / (trackWidth - fovSliderHandle.AbsoluteSize.X), 0, 1)
    ...
    local function updateHandleFromFovValue()
        ...
        local handleX = ratio * (trackWidth - fovSliderHandle.AbsoluteSize.X)
        fovSliderHandle.Position = UDim2.new(0, handleX, 0.5, -6)
    end
    
    -- [NEW] Smoothing Slider Logic
    local function updateSmoothFromHandlePosition()
        local trackWidth = smoothSliderTrack.AbsoluteSize.X
        local handleX = smoothSliderHandle.Position.X.Offset
        aimSmoothing = math.clamp(handleX / (trackWidth - smoothSliderHandle.AbsoluteSize.X), 0, 1)
        smoothValueLabel.Text = string.format("%.2f", aimSmoothing)
    end
    local function updateHandleFromSmoothValue()
        local trackWidth = smoothSliderTrack.AbsoluteSize.X
        local handleX = aimSmoothing * (trackWidth - smoothSliderHandle.AbsoluteSize.X)
        smoothSliderHandle.Position = UDim2.new(0, handleX, 0.5, -6)
    end
    ...
    fovSliderHandle.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then isDraggingFovSlider = true end end)
    smoothSliderHandle.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then isDraggingSmoothSlider = true end end)
    ...
    UserInputService.InputChanged:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseMovement then
            ...
            if isDraggingFovSlider then
                local trackStartX, handleWidth, trackWidth = fovSliderTrack.AbsolutePosition.X, fovSliderHandle.AbsoluteSize.X, fovSliderTrack.AbsoluteSize.X
                local newHandleX = mouseX - trackStartX - (handleWidth / 2)
                local clampedX = math.clamp(newHandleX, 0, trackWidth - handleWidth)
                fovSliderHandle.Position = UDim2.new(0, clampedX, 0.5, -6)
                updateFovFromHandlePosition()
            elseif isDraggingSmoothSlider then
                local trackStartX, handleWidth, trackWidth = smoothSliderTrack.AbsolutePosition.X, smoothSliderHandle.AbsoluteSize.X, smoothSliderTrack.AbsoluteSize.X
                local newHandleX = mouseX - trackStartX - (handleWidth / 2)
                local clampedX = math.clamp(newHandleX, 0, trackWidth - handleWidth)
                smoothSliderHandle.Position = UDim2.new(0, clampedX, 0.5, -6)
                updateSmoothFromHandlePosition()
            end
        end
    end)
    ...
    local function buildPlayerDropdownFrame()
        ...
        playerDropdownFrame = Instance.new("Frame", MainScreenGui) -- [FIX] Parent to MainScreenGui
        ...
        playerDropdownFrame.Position = UDim2.fromPixel(absolutePos.X, absolutePos.Y + 22)
        ...
        if aiming and aimPart and targetModel then
            if not wallCheckEnabled or isPartVisible(aimPart) then
                ...
                if silentAimEnabled then
                    ...
                    -- [MODIFIED] Replaced direct CFrame set with smoothing logic
                    local targetCFrame = CFrame.new(Camera.CFrame.Position, predictedPosition)
                    local alpha = math.max(0.01, 1 - aimSmoothing)
